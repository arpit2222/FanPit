...
ipcMain.handle('qvac:translate', async (evt, text) => {
  try {
    // Try to import the SDK. If it wasn't installed due to network errors, use a mock.
    let sdk;
    try {
      sdk = require('@qvac/sdk');
    } catch(e) {
      console.log('QVAC SDK not found, using mock implementation.');
      return '[Mock Translation]: ' + text;
    }
    
    // Using actual SDK
    const modelId = await sdk.loadModel({ modelType: "llm" });
    const response = sdk.completion({
      modelId: modelId,
      history: [
        { role: "system", content: "You are a helpful translation assistant. Translate the following text to Spanish, or if it is already in Spanish, summarize it in English. Reply only with the translation or summary." },
...
ipcMain.handle('wdk:create', async () => {
  return {
    ...
    balance: '50.00',
    currency: 'USDt (Testnet)'
  };
});

ipcMain.handle('wdk:transaction', async (evt, { to, amount }) => {
  // mock transaction signing
  return {
    txId: 'tx-mock-hash-' + Date.now(),
    status: 'success'
  };
});
